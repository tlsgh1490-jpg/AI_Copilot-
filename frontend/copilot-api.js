  ...
  async function refreshCopilot() {
    ...
    try {
      ...
      const briefDetails = brief.예상원인 && brief.영향KPI && brief.점검우선순위
        ...
        : '';
      issue.innerHTML = `<h3>주요 이상 징후</h3><p>${escapeHtml(analysis.target.label)} 최신값 ${escapeHtml(analysis.target.value)}${escapeHtml(analysis.target.unit)} · 상태 ${escapeHtml(analysis.status)}</p><div class="inline-standard"><span>현재값<b>${escapeHtml(analysis.target.value)}${escapeHtml(analysis.target.unit)}</b></span><span>기준 대비<b class="red-text">${escapeHtml(analysis.target.variance ?? '-')}</b></span></div><h3>${narrativeLabel}</h3><p>${escapeHtml(result.narrative)}</p>${briefDetails}<p class="disclaimer">데이터 기반 점검 우선순위이며 확정 원인은 아닙니다.</p>`;
